...
exports.create = (req, res) => {
  ...
  form.parse(req, (err, fields, files) => {
    ...
    // check for fileds
    const { name, author, grade, category, lang , source, year, page, publisher} = fields;
    if (!name || !author || !grade || !category || !lang || !source|| !year|| !page|| !publisher) {
      return res.status(400).json({
...
exports.decreaseQuantity = (req, res, next) => {
  let bulkOps = req.body.order.products.map((item) => {
    return {
      updateOne: {
        filter: { _id: item._id },
        update: { $inc: { lang: -item.count, sold: +item.count } },
      },
    };
  });
